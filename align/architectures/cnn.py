...
(the ``conv_stack`` block selects it); non-homogeneous activations (e.g. the
...
@register_adapter
@dataclass
class CNNAdapter(ArchitectureAdapter):
    """Recipe for plain conv->flatten->dense parameter trees.

    Emits one ``conv_stack`` block (conv channel groups, named by module
    scope path) plus one ``fcn`` dense-stack block, tied together by the
    ...
    def _discover(
        ...
        if not conv_names:
            raise ValueError(
                ...
                f"'{self.layer_root}'; use the dense_mlp adapter for pure "
                "dense trees."
            )
        if not dense_names:
            raise ValueError(
                f"No dense layers (2-D kernels) discovered under "
                f"'{self.layer_root}'; the CNN adapter needs a dense chain "
                "after the flatten."
            )
    ...
    def build_problem(self, params: Mapping[str, Any]) -> AlignmentProblem:
        ...
        DenseStackBlockAdapter(block_id="fcn", layer_paths=tuple(dense_paths)).build(
            builder
        )
