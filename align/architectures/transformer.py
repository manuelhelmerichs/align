...
delegated to block adapters:

- :class:`~align.architectures.blocks.ResidualStreamBlockAdapter`: one global
...
- :class:`~align.architectures.blocks.AttentionBlockAdapter` per block: the
  inter-head group plus per-slot qk/vo intra-head groups and the
  ``attention_block`` constraint,
- :class:`~align.architectures.blocks.DenseStackBlockAdapter` for FFN chains
...
@register_adapter
@dataclass
class TransformerAdapter(ArchitectureAdapter):
