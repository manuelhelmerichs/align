...
- the RMSNorm residual stream carries a full orthogonal symmetry: the stream
  ...
  ``stream_transforms`` for Procrustes alignment), and the exact per-channel
...
- GQA quotients the head symmetry into kv-group permutations times per-group
  ...
  :class:`~align.architectures.blocks.GQAAttentionBlockAdapter`; vo groups
...
from ..alignment.tensor_ops import _descend, _maybe_descend
...
@register_adapter
@dataclass
class ModernTransformerAdapter(ArchitectureAdapter):
    """Recipe for RMSNorm + RoPE + GQA pre-norm decoder trees.

    ``stream_transforms`` selects the modeled stream symmetry class:
    ...
    layer_root: str = ""
    stream_transforms: str = "signed_permutation"

    def __post_init__(self) -> None:
        if self.stream_transforms not in ("signed_permutation", "orthogonal"):
            raise ValueError(
                "modern_transformer stream_transforms must be "
                "'signed_permutation' or 'orthogonal', got "
                f"{self.stream_transforms!r}."
            )

    # -- discovery -------------------------------------------------------

    def _root_path(self) -> tuple[str, ...]:
        return tuple(part for part in self.layer_root.split(".") if part)
    ...
    def build_problem(self, params: Mapping[str, Any]) -> AlignmentProblem:
        ...
        ResidualStreamBlockAdapter(
            ...
            transforms=self.stream_transforms,
        ).build(builder)
        ...
        for scope in block_scopes:
            ...
            GQAAttentionBlockAdapter(
                block_id=f"{block_name}/attention",
        ...
        for index, chain in enumerate(outside_chains):
            DenseStackBlockAdapter(
                block_id="head" if index == 0 else f"head_{index}",
