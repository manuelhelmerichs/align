...
- ``python -m benchmarks`` exposes both plus real-experiment posterior
...
def _orbit_record(
    *,
    ...
    canonicalize: bool,
    seed: int,
    restrict_sinkhorn_to_plain_groups: bool = False,
) -> BenchmarkRecord:
    case = ORBIT_CASE_FACTORIES[case_name](seed=seed)
    if restrict_sinkhorn_to_plain_groups:
        plain_groups = [
            group_id
            for group_id in case.graph.group_order
            if case.graph.groups[group_id].transform_family == "permutation"
        ]
        schedule = [
            {**step, "groups": plain_groups}
            if step.get("solver") == "sinkhorn"
            else dict(step)
    ...
    metrics.update(
        {
            ...
            "function_drift_rmse": result.function_drift_rmse,
            "function_drift_relative_rmse": result.function_drift_relative_rmse,
    ...
    for seed in orbit_seeds:
        records.append(
            _orbit_record(
                ...
                canonicalize=True,
                seed=seed,
                restrict_sinkhorn_to_plain_groups=True,
            )
        )
...
DEFAULT_THRESHOLDS: dict[str, dict[str, dict[str, float]]] = {
    ...
    "orbit/rmsnorm_gqa_rope_transformer_scaled/*": {
        # The expected qk action is identity, but rotation-pair groups are
        # recovered through floating-point closed-form projections.  Preserve
        # a tight numerical allowance while discrete groups remain exact.
        "recovered_transform_error": {"max": 1e-6},
        "optimality_gap": {"max": 1e-4},
    },
    ...
    "posterior/synthetic_mlp/*": {
        ...
        "before_function_cross_barrier_mean": {"min": 0.2},
        "after_function_cross_barrier_mean": {"max": 0.05},
        "function_drift_max": {"max": 1e-4},
    },
    # High within-chain noise (0.3) leaves a real residual barrier after
    # alignment (~0.08 fast / ~0.21 full), of the order of the within-chain
    # noise floor (~0.1); the bound pins collapse toward that floor, not to
    # zero.
    "posterior/synthetic_mlp_anisotropic/*": {
        ...
        "before_function_cross_barrier_mean": {"min": 0.3},
        "after_function_cross_barrier_mean": {"max": 0.35},
        "function_drift_max": {"max": 1e-4},
    },
    ...
    "posterior/synthetic_layernorm_mha_transformer/*": {
        ...
        "before_function_cross_barrier_mean": {"min": 0.5},
        "after_function_cross_barrier_mean": {"max": 0.05},
...
_MARKDOWN_COLUMNS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    "posterior": (
        ...
        ("before_function_cross_barrier_mean", "barrier before"),
        ("after_function_cross_barrier_mean", "barrier after"),
