"""Repository-only synthetic exact-orbit benchmarks for alignment correctness.
...
def residual_conv_apply(params: ParamTree, x: jax.Array) -> jax.Array:
...
def frn_residual_conv_apply(params: ParamTree, x: jax.Array) -> jax.Array:
...
def make_frn_residual_conv_orbit_case(seed: int = 0) -> SyntheticOrbitCase:
    ...
    return SyntheticOrbitCase(
        ...
        target=target,
        inputs=inputs,
        apply_fn=frn_residual_conv_apply,
...
def layer_norm(x: jax.Array, module: Mapping[str, Any]) -> jax.Array:
...
def layernorm_mha_transformer_apply(params: ParamTree, tokens: jax.Array) -> jax.Array:
    """Forward pass matching the bundled GPT layout and semantics.

    Note the legacy component is not standard pre-LN: each LayerNorm *replaces*
    ...
    x = layer_norm(x, params["LayerNorm_0"])  # type: ignore[index]
...
def make_layernorm_mha_transformer_params(
    *,
    ...
    """Random parameters in the bundled GPT tree layout."""
...
def make_dense_params(
    *,
    ...
    """Random Flax-layout dense parameters with He fan-in-scaled init.

    Kernel std ``sqrt(2 / fan_in)`` keeps activation and function scales O(1)
    in width, so width sweeps compare geometry rather than function magnitude.
    The deterministic column signature (a per-unit anchor that breaks
    permutation ties) scales with the same std so the anchor-to-noise ratio is
    width-invariant.
    """
    ...
    for idx, (layer_key, in_dim, out_dim) in enumerate(
        ...
        column_signature = std * jnp.linspace(-0.35, 0.35, out_dim, dtype=jnp.float32)
        fcn[f"Dense_{idx}"] = {
            "kernel": std
            * jax.random.normal(weight_key, (in_dim, out_dim), dtype=jnp.float32)
            ...
            "bias": 0.14
            * std
            * jax.random.normal(bias_key, (out_dim,), dtype=jnp.float32)
...
def make_mlp_orbit_case(seed: int = 0) -> SyntheticOrbitCase:
    ...
    key = jax.random.PRNGKey(seed)
    param_key, input_key = jax.random.split(key)
    reference = make_dense_params(key=param_key, sizes=(3, 5, 4, 2))
    recipe = MLPRecipe()
    graph = recipe.build_graph(reference)
...
def make_split_concat_conv_orbit_case(seed: int = 0) -> SyntheticOrbitCase:
    ...
    target = graph.apply_transforms(
        reference,
        TransformState.from_transforms(graph, forward_perms),
    )
...
def transform_validity_error(
    ...
    for group_id, transform in transforms.items():
        ...
        matrix = _artifact_transform_matrix(transform, transform_family)
...
def recovered_transform_error(
    actual: Mapping[str, Any], expected: Mapping[str, np.ndarray], graph=None
) -> float:
    ...
    for group_id, expected_transform in expected.items():
        if group_id not in actual:
            return float("inf")
        family = graph.groups[group_id].transform_family if graph is not None else None
        actual_transform = _artifact_transform_matrix(actual[group_id], family)
...
def _artifact_transform_matrix(value: Any, family: str | None) -> np.ndarray:
    """Materialize one compact transform artifact for benchmark diagnostics."""

    array = np.asarray(value)
    if family == "permutation" or (family is None and array.ndim == 1):
        indices = array.astype(np.int64)
        return np.eye(indices.size, dtype=np.float64)[indices]
    if family == "signed_permutation":
        indices = array[0].astype(np.int64)
        signs = array[1].astype(np.float64)
        return np.eye(indices.size, dtype=np.float64)[indices] * signs[:, None]
...
def _canonicalize_if_requested(
    ...
    if case.graph.metadata.get("architecture") in {"mlp", "convnet"}:
        kwargs["activation"] = "relu"
    elif case.graph.metadata.get("architecture") == "residual_convnet":
        kwargs["activation"] = "tlu"
    canonical, _, _ = ScaleCanonicalizer().canonicalize(case.graph, params, **kwargs)
    return canonical
    ...
    *,
    ...
    metrics = AlignmentBenchmarkMetrics(
        ...
        recovered_transform_error=recovered_transform_error(
            transforms, case.expected_transforms, case.graph
        ),
