...
- **RoPE qk pair scale.** Rotary embeddings restrict the qk circuit symmetry
  ...
  bindings via :meth:`AlignmentProblem.apply_scales` with pair-tiled
...
def rms_norm_constraints(problem: AlignmentProblem) -> tuple[GraphConstraint, ...]:
...
def has_modern_transformer_plan(problem: AlignmentProblem) -> bool:
...
def qk_pair_scales(
    ...
    *,
    ...
    :meth:`AlignmentProblem.apply_scales`. Pair-constant factors commute with
...
def gqa_vo_balancing_scales(
    ...
    *,
    ...
    two. Applied through ``AlignmentProblem.apply_scales`` (value ``out``
