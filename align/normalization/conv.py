...
from ..alignment import AlignmentProblem, binding_axis_interval
from ..alignment.tensor_ops import _descend, binding_selector


def has_conv_stack_block(problem: AlignmentProblem) -> bool:
    """True if the problem contains a ``conv_stack`` block."""

    return any(block.kind == "conv_stack" for block in problem.blocks.values())
